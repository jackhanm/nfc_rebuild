...
- (instancetype)initWithUrl:(NSString *)url fileId:(NSString *)fileId delegate:(id<JKDownloadTaskDelegate>)delegate {
...
+ (instancetype)taskWithUrl:(NSString *)url fileId:(NSString *)fileId delegate:(id<JKDownloadTaskDelegate>)delegate {
  return [[JKDownloadTask alloc] initWithUrl:url fileId:fileId delegate:delegate];
}
...
- (void)resume {
  [JKDownloadSession.downloadSession resumeDownloadWithTask:self];
}

- (void)pause {
  [JKDownloadSession.downloadSession pauseDownloadWithTask:self];
}

- (void)remove {
  [JKDownloadSession.downloadSession stopDownloadWithTask:self];
}
...
-(NSString *)taskId {
  return [JKDownloadTask taskIdForUrl:self.downloadURL fileId:self.fileId];
}
...
- (NSString *)saveName {
  if (_saveName.length==0) {
    NSString *name = [JKDownloadTask taskIdForUrl:self.downloadURL fileId:self.fileId];
    NSString *pathExtension =  [JKDownloadTask getPathExtensionWithUrl:self.downloadURL];
    name = [NSString stringWithFormat:@"%@.zip",self.fileId];
    return name;
  }
  return _saveName;
}
...
- (void)setSaveName:(NSString *)saveName {
  _saveName = saveName;
  [JKDownloadSession.downloadSession saveDownloadStatus];
}

+ (NSString *)taskIdForUrl:(NSString *)url fileId:(NSString *)fileId {
  NSString *name = [JKDownloadTask md5ForString:fileId.length>0 ? [NSString stringWithFormat:@"%@-%@",url, fileId] : url];
  return name;
}
...
+ (NSString *)saveDir {
  ...
  saveDir = [saveDir stringByAppendingPathComponent:@"JKDownload/video"];
...
#pragma mark -- JKResumeData implementation
...
@interface JKResumeData()

@end

@implementation JKResumeData
...
+ (NSData *)correctResumeData:(NSData *)data
{
  ...
  NSMutableDictionary *resumeDictionary = [JKResumeData getResumeDictionary:data];
  if (resumeDictionary == nil) {
    return nil;
  }
  resumeDictionary[kResumeCurrentRequest] =  [JKResumeData correctRequestData: resumeDictionary[kResumeCurrentRequest]];
  resumeDictionary[kResumeOriginalRequest] = [JKResumeData correctRequestData:resumeDictionary[kResumeOriginalRequest]];
...
+ (NSURLSessionDownloadTask *)downloadTaskWithCorrectResumeData:(NSData *)resumeData urlSession:(NSURLSession *)urlSession {
  ...
  NSData *cData = [JKResumeData correctResumeData:resumeData];
  cData = cData ? cData:resumeData;
  NSURLSessionDownloadTask *task = [urlSession downloadTaskWithResumeData:cData];
  NSMutableDictionary *resumeDic = [JKResumeData getResumeDictionary:cData];
  if (resumeDic) {
