...
+ (UIImage *)addText:(NSString *)text fontSize:(CGFloat)fontSize angle:(CGFloat)angle endImgSize:(CGSize)endImgSize {
    ...
    for (int i=0; i<floor(wh/(font*2)); i++) {
        for (int j=0; j<ceil(wh/textW); j++) {
            [text drawInRect:CGRectMake(textW*j, textH*i, textSize.width, font+4) withAttributes:@{NSFontAttributeName:[UIFont systemFontOfSize:font], NSForegroundColorAttributeName:[UIColor orangeColor]}];
        }
    }
    // 生成水印图片
